...
(function($) {
	...
	window.bizpanda.socialOptions = {
		...
		updateButtonOrder: function(value) {
			...
			if( !value ) {
				...
				socialTabWrap.find('li:not(.sortable-placeholder):not(.factory-disabled)').each(function() {
					...
					var defaultOrderFreeButtons = [
						'facebook-like',
						'twitter-tweet',
						'google-plus'
					];

					if( window.bizpanda.socialOptions.filterDefaultOrderFreeButtons ) {
						defaultOrderButtons = window.bizpanda.socialOptions.filterDefaultOrderFreeButtons(defaultOrderFreeButtons);
					}

					if( window['sociallocker-next-build'] === 'free' && $.inArray(tabId, defaultOrderFreeButtons) >= 0 ) {
		...
		getSocialOptions: function() {
			...
			if( window.bizpanda.socialOptions.filterOptions ) {
				options = window.bizpanda.socialOptions.filterOptions(options);
			}
