<?php
	...
	add_filter('bizpanda_social-locker_type_metaboxes', 'opanda_socail_locker_metaboxes', 10, 1);
	...
	function opanda_social_locker_activation($plugin, $helper)
	{
		...
		$wpdb->query("UPDATE {$wpdb->posts} SET post_title='" . __('Social Locker', 'bizpanda-social-locker') . "', post_name='opanda_default_social_locker' WHERE post_name='default_sociallocker_locker'");
		...
		$helper->addPost('opanda_default_social_locker_id', array(
			...
			'post_title' => __('Social Locker (default)', 'bizpanda-social-locker'),
			'post_name' => 'opanda_default_social_locker'
		), array(
			'opanda_item' => 'social-locker',
			'opanda_header' => __('This content is locked', 'bizpanda-social-locker'),
			'opanda_message' => __('Please support us, use one of the buttons below to unlock the content.', 'bizpanda-social-locker'),
	...
	add_action('bizpanda_after_activation', 'opanda_social_locker_activation', 10, 2);
	...
	function opanda_register_social_locker_themes()
	{
		...
		if( !onp_build('free') ) {
			...
			OPanda_ThemeManager::registerTheme(array(
				...
				'hint' => sprintf(__('This theme is available only in the <a href="%s" target="_blank">premium version</a> of the plugin', 'bizpanda-social-locker'), opanda_get_premium_url(null, 'themes')),
				'items' => array('social-locker')
			));
	...
	add_action('bizpanda_register_themes', 'opanda_register_social_locker_themes');
	 ...
	 * @since 1.0.0
	 */
	function bizpanda_help_page_usage_what_is_social_locker($manager)
	{
		require BIZPANDA_SOCIAL_LOCKER_DIR . '/admin/help/what-is-it.php';
	}

	add_action('bizpanda_help_page_what-is-social-locker', 'bizpanda_help_page_usage_what_is_social_locker');
	 ...
	 * @since 1.0.0
	 */
	function bizpanda_help_page_usage_example_social_locker($manager)
	{
		require BIZPANDA_SOCIAL_LOCKER_DIR . '/admin/help/usage-example.php';
	}

	add_action('bizpanda_help_page_usage-example-social-locker', 'bizpanda_help_page_usage_example_social_locker');
	 ...
	 * @since 1.0.0
	 */
	function bizpanda_help_page_other_notes_social_locker($manager)
	{
		require BIZPANDA_SOCIAL_LOCKER_DIR . '/admin/help/other-notes.php';
	}

	add_action('bizpanda_help_page_other-notes-social-locker', 'bizpanda_help_page_other_notes_social_locker');
	...
	function opanda_social_locker_stats_screens($screens)
	{
		...
		$screens = array(
			...
			'summary' => array(
				'title' => __('<i class="fa fa-search"></i> Summary', 'bizpanda-social-locker'),
				'description' => __('The page shows the total number of unlocks for the current locker.', 'bizpanda-social-locker'),
			...
			'channels' => array(
				'title' => __('<i class="fa fa-search-plus"></i> Detailed', 'bizpanda-social-locker'),
				'description' => __('The page shows which ways visitors used to unlock the content.', 'bizpanda-social-locker'),
		...
		if( !onp_build('free') ) {
			...
			$screens['skips'] = array(
				'title' => __('<i class="fa fa-tint"></i> Skips', 'bizpanda-social-locker'),
				'description' => __('The chart shows how many users skipped the locker by using the Timer or Close Icon, comparing to the users who unlocked the content.', 'bizpanda-social-locker'),
	...
	add_filter('bizpanda_social-locker_stats_screens', 'opanda_social_locker_stats_screens', 10, 1);