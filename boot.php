<?php
	...
	if( is_admin() ) {
		require BIZPANDA_SOCIAL_LOCKER_DIR . '/admin/boot.php';
	}

	global $bizpanda;

	load_textdomain('bizpanda-social-locker', BIZPANDA_SOCIAL_LOCKER_DIR . '/langs/bizpanda-social-locker-' . get_locale() . '.mo');
	...
	function opanda_register_social_locker($items)
	{
		...
		$title = __('Social Locker', 'bizpanda-social-locker');
		...
		if( onp_build('free') ) {
			...
			$items['social-locker'] = array(
				...
				'description' => __('<p>Asks users to "pay with a like" or share to unlock content.</p><p>Perfect way to get more followers, attract social traffic and improve some social metrics.</p>', 'bizpanda-social-locker'),
	...
	add_filter('bizpanda_items', 'opanda_register_social_locker', 1);
	...
	function opanda_social_locker_options($options, $id)
	{
		...
		$options['socialButtons']['facebook'] = array(
			...
			'version' => opanda_get_option('facebook_version', 'v2.8'),
			'like' => array(
				'url' => $fbLikeUrl,
		...
		$allowedButtons = apply_filters('bizpanda_social-locker_allowed_buttons', $allowedButtons);
	...
	add_filter('bizpanda_social-locker_item_options', 'opanda_social_locker_options', 10, 2);
	...
	add_action('bizpanda_request_assets_for_social-locker', 'opanda_social_locker_assets', 10, 4);
	...
	FactoryShortcodes000::register('OPanda_SocialLockerShortcode', $bizpanda);